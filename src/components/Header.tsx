...
import { fetchServices, richTextToPlainText, Service } from '@/lib/strapi'
...
const Header: React.FC = () => {
	...
	return (
		<header className='relative z-50 flex min-h-[70px] border-amber-700 border-b bg-amber-900 tracking-wide'>
									...
									{loading ? (
										...
										<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2'>
											{services.map((service) => (
												<a
													key={service.slug}
													href={`/${i18n.locale || 'en'}/services/${service.slug}`}
													className='block rounded px-3 py-2 font-medium text-[14px] text-amber-50 transition-colors hover:bg-amber-700 hover:text-amber-100'
			...
			{/* Mobile Menu */}
			<div
				className={`fixed top-0 right-0 z-50 h-full w-80 transform overflow-y-auto bg-amber-900 shadow-lg transition-transform duration-300 ease-in-out lg:hidden ${
					...
					<div className='border-amber-700 border-b'>
						...
						{isServicesOpen && (
							<div className='max-h-64 space-y-1 overflow-y-auto pb-3 pl-4'>
								{loading ? (
									<div className='py-2 text-amber-200 text-sm'>
										Loading services...
									</div>
								) : services.length > 0 ? (
									services.map((service) => (
